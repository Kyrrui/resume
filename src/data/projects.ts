...
export const projects: Record<string, ProjectDetails> = {
  ...
  "kin-quilt": {
    title: "KinQuilt.com",
    subtitle: "Solo Engineer · 2026–Present",
    url: "https://kinquilt.com",
    logo: "/kin_quilt.png",
    logoShape: "tile",
    description:
      "Genealogy research workspace — a Vite + React + TypeScript SPA paired with a Chrome MV3 extension that aggregates a person's records across FamilySearch, WikiTree, FindAGrave, Ancestry, and Chronicling America into a single triage view, then drafts a cited biography with an LLM.",
    highlights: [
      "Solo-built v0 alpha with continuous deploy to Netlify; first real-world WikiTree profile updated via a Kin Quilt biography within 3 weeks of nights-and-weekends work.",
      "Chrome MV3 extension orchestrates hidden-tab scraping across 5 logged-in genealogy sites and relays results back to the SPA over a 3-hop message channel.",
      "LLM biography generator (Kimi K2 via OpenRouter) with explicit citation rules, prompt-injection hardening, and deterministic post-validation that catches hallucinated facts before they reach the user.",
      "Export pipeline: WikiTree wiki-markup → .docx with native Word footnotes, hyperlinked Sourcer templates, and copy-to-clipboard for Google Docs via Drive's .docx import.",
    ],
    details: [
      "Service worker + content scripts on five external sites; message passing across window.postMessage ↔ chrome.runtime.sendMessage ↔ chrome.tabs.sendMessage boundaries.",
      "Hidden-tab scrape orchestration with taskbar-flash mitigation tuned across OS configurations; defensive matchers that survive source-site redesigns.",
      "MutationObserver + IntersectionObserver for lazy-load handling and hydration race-condition fixes on heavy SPA targets.",
      "Prompt engineering: XML delimiters, instruction firewalling, sanitization; post-generation validation runs citation cross-check, fact verification, and unverified-phrase warnings. Token-budget tuning + Haiku→Kimi swap for cost.",
      "State + data modeling: Zustand persistent stores with versioned migrations; fuzzy-match dedupe (strict / maiden-married / first-name clustering); union-find for relative grouping; time-series snapshots for the resolve wizard.",
      "Backend: Netlify serverless functions (Node), Netlify Blobs for rate-limit storage, OpenRouter integration with timeouts + retry, multi-tier rate limiting.",
      "API integration: WikiTree REST (proxied for CORS), Library of Congress Chronicling America, Google Drive ingestion via .docx.",
      "Ops: continuous deploy to main, defensive testing against real exported user data, diagnostic-log instrumentation for hard-to-reproduce bugs (browser flicker, listener races). ~94 extension versions shipped with no regression making it past one deploy cycle.",
    ],
    stack: [
      "React 19",
      "TypeScript",
